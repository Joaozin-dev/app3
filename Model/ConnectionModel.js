			...
			if(array[i].Socket === socket.id){
				...
				for(var e = 0; e < jogArray.length; e++){
					if(jogArray[e].socket === socket.id){
						socket.to(array[i].Socket).emit('player-disconnected',{socket: socket.id,player:jogArray[e].player.uid});
						db.ref(`conexao/${array[i].ID}/Jogadores/${jogArray[e].ID}`).remove();
					}
				}
			...
			if(usucode === dbcode){
				...
				if(!SExist(jogarray,socket.id)){
                     // SE NÃO ESTIVER ADICIONA ELA
          			...
          			socket.to(array[i].Socket).emit('new-player',{socket: socket.id,player: data.player});
                 }
			}
		}
    },
        ...
        if(item.length > 0 && item !== 'undefinded'){
            	for(var i = 0; i < item.length; i++){
            		if(item[i].Socket === socket.id){
            			db.ref(`conexao/${item[i].ID}/Jogadores`).once('value',(controllers)=>{
